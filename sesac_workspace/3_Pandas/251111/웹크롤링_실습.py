...
# 이미지 url 가져오기
product_info = bs.select_one("div.row div.col-md-4.col-xl-4.col-lg-4")
'''pd_img = bs.select_one("img[itemprop='image']")
print(pd_img.get("src"))'''
pd_img = product_info.select_one("img.img-fluid")["src"]
print(pd_img)
# 상품 이름 가져오기
...
pd_price = product_info.select_one("h4 span").get_text(strip=True)
print(pd_price)
...
pd_review = product_info.select_one("div.ratings p.review-count span").get_text(strip=True)
print(pd_review)

#별의 개수(선택)
'''pd_star = bs.select_one("p[data-rating]")
print(pd_star.get("data-rating"))'''
pd_star = product_info.select_one("div.ratings p:nth-child(2)")["data-rating"]
print(pd_star)
...
pd_link = product_info.select_one("h4 a.title")["href"]
print(pd_link)